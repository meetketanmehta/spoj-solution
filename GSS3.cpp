...
struct con update(ll s, ll e, ll c, ll index, ll n){
    if(s==e){
        st[c].sum+=n;
        st[c].prefixsum+=n;
        st[c].suffixsum+=n;
    ...
    if(in_range(s,e,index)){
        ll mid = middle(s,e);
        struct con leftc, rightc;
        if(in_range(s,mid,index)){
            leftc = update(s, mid, left(c), index, n);
            rightc = st[right(c)];
        }
        if(in_range(mid+1,e,index)){
            rightc = update(mid+1, e, right(c), index, n);
            leftc = st[left(c)];
        }
        st[c].prefixsum = max(leftc.allsum + rightc.prefixsum, leftc.prefixsum);
        st[c].suffixsum = max(leftc.suffixsum + rightc.allsum, rightc.suffixsum);
